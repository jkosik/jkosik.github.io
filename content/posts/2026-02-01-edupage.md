...
I changed the Student ID to a random, non-existent value: **StudentXYZ**. Although the browser failed to render the response properly, the server still returned sensitive data - specifically, the complete list of all Edupage users for the tested school. In total, around 1,000 unique records with IDs, names and banking details related to students, parents and even the teachers.
...
Luckily, we can do the same by forcing victim opening a properly designed HTML web form, bypassing all the controls. Web form fields hold the parameters which would be normally part of a legit victim's HTTP POST request.
