...
- Neur network model → a network of many parameters (weights, biases).
...
Now we can introduce one more term - `tensor`. Tensor is a naming for all inputs, outputs and weights. I.e. the data structures of various dimensions which float across the neural network.
...
Keyword spotting
- "Speech Commands" Dataset - Google (Pete Warden, Google brain)
- 1k-4k samples of selected word spoken by thosands of volunteers
- soudn wave for yes and no looks similarly - how to find unique critical features to hook on?
    - decompose sound signal, so that it does not matter how loudly we say the word (Fourier transform) = REMOVE NOISE, ETC.1
    - not using sound wave, rather, heavy preprocess first
    - classifying based on interesting features
    - app may allow tuning sensitivity on the fly

- accuracy, vs latency vs bandwidth
- keyword spotting - patented

Parameters - number of weigths - in Dense fully conected - for 2 layers of 4 and 5 neurrons => 4X5 + 5(bias) = 25 params. - number of links + number of neurons (biases)

- keyword spotting on tiny ml model. further instructions pushed to cloud or pre-precessed in CPU - cascaded architecture

## Googl sppech trainign
- The Audio Preprocessor model converts raw audio samples into a spectrographic feature.
    - https://github.com/tensorflow/tflite-micro/tree/main/tensorflow/lite/micro/examples/micro_speech
- Audio Preprocessing models providing int8 and float32 output, ready for use with the Micro Speech model.
- Micro speech model: The input to the model is 49 spectrographic features, each feature consisting of 40 channels of data. The features are generated by the Audio Preprocessor model.



---

## Manage laptop using sound orders
Google Colab

1. Collect data, label data, clean the data, augment data (extend)
2. Define mode, train, measure, iterate
3. Model size optimisation (quantisation, pruning...), security and privacy,
    improve latency, lower accurancy...
