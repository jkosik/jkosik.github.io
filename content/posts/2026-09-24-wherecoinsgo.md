...
I built [wherecoinsgo.org](https://wherecoinsgo.org) to demonstrate benefits of blockchain in the area of donation and transparency account platforms. Conventional systems oftentimes offer an illusion of transparency and provide only one layer deep visibility and mostly only on the sender side, not tracking expenditures at all. Moreover, the identity of senders and receivers is mostly very vague and not directly trusted, oftentimes just an arbitrarily written name or bank account number with no attribution to the origin of the money.
...
#### 1. Fund raising
Offers one-click creation of your own Wherecoinsgo **projects** with **shareable paylinks** and QR codes, **balance views** and **transaction history** for transparent fund raising.

{{< figure src="images/wherecoinsgo/paylink.png" >}}

#### 2. User-friendly blockchain explorer
Acts as an easy-to-use public interface for browsing the blockchain and transactions related to the submitted crypto wallet on Bitcoin, Ethereum and [Ink](https://inkonchain.com/) blockchain. Suspicious intermediaries can also be spotted using an integrated [sanction list](https://ofac.treasury.gov/).

{{< figure src="images/wherecoinsgo/diagram.png" >}}

On top of these features, you can build charity programs, donation services, transparency accounts for public funding and more.
...
That can be a surprising detail for beginners looking for "lost" funds.

## Blockscout and Mempool
To interact with the blockchain and get the transaction data, we could run our own blockchain node and index all transactions for later lookups OR we could utilise existing **blockchain explorers** with mature API endpoints. The second option is definitely much better.

For Bitcoin, I traditionally selected [mempool.space](https://mempool.space) and the rest of the lookups are funnelled through [Blockscout](https://www.blockscout.com/). Blockscout offers a nice advantage of supporting a wide range of blockchains which simplifies the platform architecture and makes the code slimmer and more unified. We do not have to use too many 3rd-party API endpoints. All the heavy lifting is done by Blockscout and expanding the product to further blockchains will be very easy.

## What Wherecoinsgo is not
[wherecoinsgo.org](https://wherecoinsgo.org) is not a forensics tool. I say that to myself when the transaction diagram looks too neat. The application always relies on underlying data and does not hide the fact that it is powered by Mempool and Blockscout, and advanced users can still use direct hyperlinks and dive as deep as needed in the underlying blockchain explorers.
...
- appealing fund-raising capabilities by embedding donation buttons into your website, shareable paylinks or payment via QR code.
